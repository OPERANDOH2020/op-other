...
docker run -d -p 389:389 -p 636:636 --name openldap --dns $DNS_IP --env LDAP_DOMAIN=nodomain --env LDAP_ORGANISATION=nodomain --env HOSTNAME=integration.operando.dmz.lab.esilab.org --env LDAP_TLS_VERIFY_CLIENT=allow registry.devops.operando.esilab.org:5000/operando/eu.operando.core.as.openldap.server:ALPHA
