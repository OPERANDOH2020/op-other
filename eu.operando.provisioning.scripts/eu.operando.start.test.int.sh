...
sudo docker run -i -t --name eu.operando.core.ae.server.test.01 --dns $DNS_IP registry.devops.operando.esilab.org:5000/operando/eu.operando.core.ae.server.test.01:ALPHA
sudo docker run -i -t --name eu.operando.core.pdb.server.test.01 --dns $DNS_IP registry.devops.operando.esilab.org:5000/operando/eu.operando.core.pfb.server.test.01:ALPHA
sudo docker run -i -t --name eu.operando.core.yellowpages.server.test.01 --dns $DNS_IP -e "MYSQL_DB_HOST=mysql.integration.operando.lan.esilab.org" -e "MYSQL_DB_NAME=operando_dan" -e "MYSQL_DB_PASSWORD=root" -e "MYSQL_DB_USER=root" registry.devops.operando.esilab.org:5000/operando/eu.operando.core.bda.server.test.01:ALPHA
